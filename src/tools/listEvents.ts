...
export function registerListEventsTool(server: McpServer, client: OmadaClient): void {
    server.registerTool(
        'listEvents',
        {
            description: 'List paginated events for a site (alerts, warnings, system events). Optionally filter by module (System, Device, Client).',
            inputSchema: listEventsSchema.shape,
        },
        wrapToolHandler('listEvents', async ({ siteId, page, pageSize, timeStart, timeEnd, module }) =>
            toToolResult(await client.listEvents(siteId, page, pageSize, timeStart, timeEnd, module))
        )
    );
}
